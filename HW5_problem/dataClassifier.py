...
def readCommand( argv ):
  ...
  parser.add_option('-c', '--classifier', help=default('The type of classifier'), choices=['mostFrequent', 'nb', 'naiveBayes', 'GDA', 'logisticRegression', 'lr', 'neuralNetwork', 'nn'], default='mostFrequent')
  ...
  elif(options.classifier == "neuralNetwork" or options.classifier=="nn"):
    classifier = neuralNetwork.NeuralNetworkClassifier(legalLabels, "NeuralNetwork", 123)
...
def runClassifier(args, options):
  ...
  if options.classifier == "GDA" or options.classifier == "LR":
    ...
    validationData = basicFeatureDataToNumpyArray(map(featureFunction, rawValidationData)).astype(np.float32)
